...
const init = () => {
  const timeline = gsap.timeline({
    onComplete: autoScroll, // Appeler autoScroll après la première animation
  });
...
const draggable = () => {
  ...
  Draggable.create(".items", {
    ...
    onDragStart: function () {
      start = this.rotation;
      gsap.killTweensOf(".items"); // Arrêter l'animation automatique lors du drag
    },
    onDragEnd: function () {
      ...
      autoScroll(); // Relancer l'animation automatique après le drag
    },
  });
};
...
const autoScroll = () => {
  gsap.to(".items", {
    rotation: `-=360`,
    duration: 50, // Ajuster la durée selon la vitesse souhaitée
    ease: "linear",
    repeat: -1,
  });
};
